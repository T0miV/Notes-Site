...
export const addNote = async (req: Request, res: Response) => {
  ...
  try {
    const result = await query(
      'INSERT INTO notes (title, text, timestamp, color, user_id) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [title, text, timestamp, color, userId]
    );
...
export const updateNote = async (req: Request, res: Response) => {
  ...
  try {
    const result = await query(
      'UPDATE notes SET title = $1, text = $2, color = $3 WHERE id = $4 RETURNING *',
      [title, text, color, noteId]
    );
